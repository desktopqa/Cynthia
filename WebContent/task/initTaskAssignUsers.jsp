...
<%
	...
	if(userSet.size() == 0){
		...
		if(data != null){
			if(data.getStatusId().toString().equals(statId.toString())){//当前是编辑动作
				firstUser = data.getAssignUsername();
			}
			
			if(firstUser == null){
				if(userSet.contains(data.getAssignUsername())){
					firstUser = data.getAssignUsername();
				}
				...
				for(int i = data.getChangeLogs().length - 1; i >= 0; i--){
					if(data.getChangeLogs()[i].getActionId() != null && userSet.contains(data.getChangeLogs()[i].getCreateUser())){
						if(firstUser == null){
							firstUser = data.getChangeLogs()[i].getCreateUser();
						}
						else if(!firstUser.equals(data.getChangeLogs()[i].getCreateUser())){
							firstUser = null;
						}
		...
		for(String user : userSet){
			if(firstUser == null || !user.equals(firstUser)){
