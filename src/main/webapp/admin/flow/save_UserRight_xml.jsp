...
<%
	...
	String[] userRightArray = (String[])ArrayUtil.format(request.getParameterValues("userRight"), new String[0]);
	for(String userRight : userRightArray)
	{
		String user = userRight.split("\\|")[0];
		boolean right = Boolean.parseBoolean(userRight.split("\\|")[1]);
		
		if(right)
			flow.addRight(user, templateId, roleId);
		else
			flow.removeRight(user, templateId, roleId);
	}
