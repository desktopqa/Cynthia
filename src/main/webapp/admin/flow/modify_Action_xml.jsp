...
// 	flow = (Flow)flow.clone();
	...
	Action action = flow.getAction(id);
	if(action == null)
	{
	...
	if(request.getParameter("beginStatId") != null)
		action.setBeginStatId(DataAccessFactory.getInstance().createUUID(request.getParameter("beginStatId")));
	else
		action.setBeginStatId(null);
	
	action.setEndStatId(DataAccessFactory.getInstance().createUUID(request.getParameter("endStatId")));
