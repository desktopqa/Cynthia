...
K.options = {
	...
	//],
	items : [
		'justifyleft', 'insertorderedlist', 'insertunorderedlist', 'indent', 'outdent',
		'formatblock', 'fontname', 'fontsize', '|', 'forecolor', 'hilitecolor', 'bold',
		'strikethrough', '|', 'image',
		'insertfile', 'table', 'hr',
		'link', 'unlink','|', 'fullscreen', 'removeformat'
	],
...
KEditor.prototype = {
	...
	create : function() {
		...
		container.removeClass('container')
			.addClass('ke-container ke-container-' + self.themeType).css('width', "100%");
		if (fullscreenMode) {
			container.css({
				position : 'absolute',
				left : 0,
...
_plugin('core', function(K) {
	...
	self.afterCreate(function() {
		...
		{
			var returnUrl = imgData;
			if(imgData.indexOf(",")>0)
			{
				var base64Data = imgData.substring(imgData.indexOf(",") +1);
				var formData = "imgData="+encodeURIComponent(base64Data);
				$.ajax({
					url : base_url + 'task/uploadEditorImg.jsp',
					type : 'POST',
					dataType:'json',
					async : false,
					data : formData,
		...
		function moveDragData(imgSrc)
		{
			var fileHtml = "";
			for(var i = 0; i< imgSrc.length; i++)
			{
				fileHtml += '<img src="'+uploadImg(imgSrc[i])+'"/>';
			}
			...
			var html = fileHtml === ""?div[0].innerHTML:fileHtml;
			div.remove();
			if (html === '') {
				return;
			}
		...
		function movePastedData(imgSrc) {
			var fileHtml = "";
			for(var i=0;i<imgSrc.length;i++)
			{
				fileHtml += '<img src="'+uploadImg(imgSrc[i])+'"/>';
		...
		K(doc.body).bind('paste', function(e){
			...
			try
			{
			 var orignEvent = e.event;
			 var clipboardData = orignEvent.clipboardData  ||  orignEvent.originalEvent.clipboardData;
		...
		if($)
		{
			$(doc.body).on('dragenter dragover', false).on('drop', function (e) {
				initParams();
